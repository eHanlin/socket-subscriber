...
/***/ function(module, exports, __webpack_require__) {
	...
	var _GeneralSocketClient = __webpack_require__(4);
	
	var _GeneralSocketClient2 = _interopRequireDefault(_GeneralSocketClient);
	
	var _Observer = __webpack_require__(5);
	
	var _Observer2 = _interopRequireDefault(_Observer);
	
	var _Idle = __webpack_require__(75);
	...
	var _Channel = __webpack_require__(76);
	...
	function buildRoomId(type, id) {
	  ...
	  return _Config.ROOM + '.' + type + '.' + id;
	}
	
	var SocketSubscriber = (_dec = (0, _Idle2.default)(), _dec2 = _Idle2.default.reset, _dec(_class = (_class2 = function () {
	  function SocketSubscriber(host) {
	    var _this = this;
	    ...
	    var _ref$retryTime = _ref.retryTime;
	    var retryTime = _ref$retryTime === undefined ? 10000 : _ref$retryTime;
	    var _ref$retryCount = _ref.retryCount;
	    var retryCount = _ref$retryCount === undefined ? -1 : _ref$retryCount;
	    var _ref$idleTime = _ref.idleTime;
	    var idleTime = _ref$idleTime === undefined ? 600000 : _ref$idleTime;
	    var _ref$incomingHeartbea = _ref.incomingHeartbeat;
	    var incomingHeartbeat = _ref$incomingHeartbea === undefined ? 1000 : _ref$incomingHeartbea;
	
	    _classCallCheck(this, SocketSubscriber);
	
	    _Observer2.default.call(this);
	
	    this._client = new _GeneralSocketClient2.default(host, {
	      retryTime: retryTime,
	      retryCount: retryCount,
	      incomingHeartbeat: incomingHeartbeat
	    });
	    this._connectPromise = this._client.connect();
	    this._client.on(_EventType.CONNECT, this._onConnect.bind(this));
	    this._client.on(_EventType.CLOSE, this._onClose.bind(this));
	    this.debug(debug);
	    this.id = random.string();
	    this._connectPromise.then(function () {
	      return _this._countIdle(idleTime);
	    });
	  }
	
	  _createClass(SocketSubscriber, [{
	    key: '_onConnect',
	    value: function _onConnect(evt) {
	
	      this.trigger(_EventType.CONNECT, null, { headers: evt.headers });
	    }
	  }, {
	    key: '_onClose',
	    value: function _onClose() {
	
	      this.trigger(_EventType.CLOSE);
	    }
	  }, {
	    key: 'debug',
	    ...
	    key: 'sendRoom',
	    value: function sendRoom(type, id, data, headers) {
	      var _this2 = this;
	
	      var roomId = buildRoomId(type, id);
	      var label = null;
	
	      if (typeof data === 'string') {
	        label = data;
	        data = headers || {};
	        headers = (arguments.length <= 4 ? undefined : arguments[4]) || {};
	      }
	
	      this.ready().then(function () {
	        return _this2._client.send(roomId, data, label, headers);
	      });
	    }
	  }, {
	    key: 'room',
	    value: function room(type, id) {
	      var _this3 = this;
	
	      var roomId = buildRoomId(type, id);
	      var channel = _Channel2.default.getInstance(this, roomId);
	
	      this.ready().then(function () {
	        return _this3._client.subscribe(roomId);
	      });
	      return channel;
	    }
	  }, {
	    key: 'exitRoom',
	    value: function exitRoom(type, id) {
	      var _this4 = this;
	
	      var roomId = buildRoomId(type, id);
	      this.ready().then(function () {
	        _this4._client.unsubscribe(roomId);
	        _Channel2.default.clear(_this4, roomId);
	      });
	    }
	  }, {
	    key: 'date',
	    value: function date() {
	      ...
	      var id = _Config.TIMER_ID;
	      var channel = _Channel2.default.getInstance(this, id);
	
	      this.ready().then(function () {
	        return _this5._client.subscribe(id);
	      });
	      return channel;
	    }
	  }, {
	    key: 'getSockJSSessionId',
	    value: function getSockJSSessionId() {
	...
	}(), (_applyDecoratedDescriptor(_class2.prototype, 'sendRoom', [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, 'sendRoom'), _class2.prototype)), _class2)) || _class);
	
	
	(0, _inherits2.default)(SocketSubscriber, _Observer2.default);
...
/***/ function(module, exports) {
	...
	var HOST_PATH = exports.HOST_PATH = '/topic';
	var TIMER_ID = exports.TIMER_ID = 'date.now';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _sockjsClient = __webpack_require__(9);
	
	var _sockjsClient2 = _interopRequireDefault(_sockjsClient);
	
	var _stomp = __webpack_require__(73);
	...
	function _subscribe(socketClient, id) {
	
	  var subscriber = socketClient._frame.headers.session;
	
	  socketClient._client.subscribe(_Config.HOST_PATH + '/' + id, function (resp) {
	...
	function _unsubscribe(socketClient, id) {
	
	  socketClient._client.unsubscribe(id);
	}
	
	function GeneralSocketClient(host) {
	  var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
	
	  var _ref$autoConn = _ref.autoConn;
	  var autoConn = _ref$autoConn === undefined ? true : _ref$autoConn;
	  var _ref$retryTime = _ref.retryTime;
	  var retryTime = _ref$retryTime === undefined ? 1000 : _ref$retryTime;
	  var _ref$retryCount = _ref.retryCount;
	  var retryCount = _ref$retryCount === undefined ? -1 : _ref$retryCount;
	  var _ref$incomingHeartbea = _ref.incomingHeartbeat;
	  var incomingHeartbeat = _ref$incomingHeartbea === undefined ? 1000 : _ref$incomingHeartbea;
	...
	GeneralSocketClient.prototype = {
	  ...
	  _initConnection: function _initConnection() {
	    ...
	    var _sessionId = this._sessionId = random.string();
	    var sockjs = this.sockjs = new _sockjsClient2.default(this.host, null, { sessionId: function sessionId() {
	        return _sessionId;
	      }, heartbeatTimeout: 1000, server_heartbeat_interval: 300 });
	
	    sockjs.addEventListener(_EventType.CLOSE, this._onClose.bind(this));
	    sockjs.addEventListener(_EventType.OPEN, this._onOpen.bind(this));
	
	    this._client = _stomp.Stomp.over(sockjs);
	    //this._client.heartbeat.outgoing = 100;
	    this._client.heartbeat.incoming = this._incomingHeartbeat;
	    this._client.debug = function (text) {
	      if (_this._isDebugging) console.log(text);
	
	      return _this._isDebugging;
	    };
	  },
	
	  _resetStates: function _resetStates() {
	    this._currentRetryCount = 0;
	  },
	
	  _incRetryCount: function _incRetryCount() {
	    this._currentRetryCount++;
	  },
	
	  _isNeedReConnection: function _isNeedReConnection() {
	  ...
	  _onClose: function _onClose() {
	    ...
	    if (this._isNeedReConnection()) setTimeout(function () {
	  ...
	  _onConnect: function _onConnect(frame) {
	    this._frame = frame;
	    this._firstConnected = true;
	    this.trigger(_EventType.CONNECT, null, { headers: frame.headers });
	  },
	  ...
	  _reconnect: function _reconnect() {
	    ...
	    this.connect(this._lastLogin, this._passcode, true).then(function () {
	      for (var name in subscriptions) {
	        _subscribe(this, name);
	      }
	    }.bind(this));
	  },
	
	  connect: function connect(login, passcode) {
	    var _this3 = this;
	
	    var _internal = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
	    ...
	    return new Promise(function (resolve, reject) {
	      return _this3._client.connect(login, passcode, function (frame) {
	        return _this3._onConnect(frame) || resolve();
	      }, function () {
	  ...
	  disconnect: function disconnect() {
	    ...
	    return new Promise(function (resolve, reject) {
	      return _this4._client.disconnect(resolve);
	    });
	  },
	  ...
	  send: function send(id, data) {
	    ...
	    var subscriber = this._frame.headers.session;
	    var sendHeaders = { subscriber: subscriber, id: id };
	
	    for (var headerName in headers) {
	      sendHeaders[headerName] = headers[headerName];
	    }this._client.send(_Config.HOST_PATH + '/' + id, sendHeaders, createBodyStr(data, label));
	  },
	
	  subscribe: function subscribe(id) {
	    var subscriptions = this._subscriptions;
	
	    if (!subscriptions[id]) _subscribe(this, id);
	...
	(0, _inherits2.default)(GeneralSocketClient, _Observer2.default);
	
	exports.default = GeneralSocketClient;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ObserverEvent = __webpack_require__(6);
	
	var _ObserverEvent2 = _interopRequireDefault(_ObserverEvent);
	...
	function getListenersByName(observer, name) {
	  var listenerArr = observer._eventListeners[name];
	
	  return listenerArr || [];
	}
	
	function appendListenerByName(observer, name, func) {
	  var listenerArr = getListenersByName(observer, name);
	...
	function removeListenerByName(observer, name, func) {
	  if (func == null) {
	    ...
	    var listenerArr = getListenersByName(observer, name);
	    var index = listenerArr.indexOf(func);
	...
	function runListener(listener, name, data, label, opts, args) {
	  listener.call(null, new _ObserverEvent2.default(name, data, label, opts));
	}
	...
	Observer.prototype = {
	  ...
	  trigger: function trigger(name) {
	    ...
	    var evtType = _name$split2[0];
	    var label = _name$split2[1];
	
	    var evtListenerArr = getListenersByName(this, evtType) || [];
	    var listenerArr = getListenersByName(this, name) || [];
	
	    evtListenerArr.forEach(function (listener) {
	      return runListener(listener, name, data, label, opts, args);
	    });
	    label && listenerArr.forEach(function (listener) {
	      return runListener(listener, name, data, label, opts, args);
	    });
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	function onIdle() {
	  this.trigger && this.trigger(_EventType.IDLE);
	}
	
	var idleMethods = {
	
	  _countIdle: function _countIdle(time) {
	    var idleInfo = this._idleInfo = { time: time };
	    this.__resetIdle();
	  },
	
	  __resetIdle: function __resetIdle() {
	    var _this = this;
	
	    var idleInfo = this._idleInfo;
	
	    if (this._idleInfo) {
	      clearTimeout(idleInfo.id);
	      idleInfo.id = setTimeout(function () {
	        return onIdle.call(_this);
	      }, idleInfo.time);
	    }
	  }
	...
	function decorateClass(target) {
	  ...
	  for (var name in idleMethods) {
	    target.prototype[name] = idleMethods[name];
	  }
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	Channel.prototype.on = function () {
	  ...
	  if (args.length > 1) {
	    ...
	    func = args[0];
	  }var name = _Observer2.default.buildName(id, type);
	...
	Channel.prototype.off = function () {
	  ...
	  if (args.length > 1) {
	    ...
	    var name = _Observer2.default.buildName(id, type);
	    var eventNameIndex = this._eventNames.indexOf(name);
	    ...
	    this._eventNames.forEach(function (name) {
	      return _this._socketSubscriber._client.off(name);
	    });
	    this._eventNames = {};
	  }
	...
	Channel.clear = function (socketSubscriber, id) {
	  var socketSubscriberId = socketSubscriber.id;
	  var key = buildKey(socketSubscriberId, id);
	
	  if (registerMap[key]) {
	    var channel = Channel.getInstance(socketSubscriberId, id);
	    channel.off();
